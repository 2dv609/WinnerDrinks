...
describe('Test suite for game module party', () => {
  ...
  /* --------------------------------- */
    ...
    test('Game module trivia should use props gameService, gameEvent and currentPlayers', () => {
      ...
      ReactDOM.render(<Trivia currentPlayers={currentPlayer} gameService={gameServiceMock} gameEvent={triviaQuestionMock} />, div)
    })
    ...
    test('T1.MT.UI.1: Trivia game events should contain 1 active game participant\'s name', () => {
      const { getByTestId } = render(<Trivia currentPlayers={currentPlayer} gameService={gameServiceMock} gameEvent={triviaQuestionMock} />)
    ...
    test('T1.MT.UI.3: Select a trivia event winner should trigger transition to next game round, button 0', () => {
      const { getByTestId } = render(<QuestionCard answer={formatAPIResponseString('mockAnswer')} handleAnswer={handleAnswerMock} key={uuidv1()} />)
